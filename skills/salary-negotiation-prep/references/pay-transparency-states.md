...
| Maryland | Oct 1, 2024 | 15+ employees | |
| New Jersey | 2025 (phased) | 10+ employees | Verify current phase/threshold. |
| Hawaii | Jan 1, 2024 | ~50+ employees | Verify current threshold. |
| Vermont | Jul 1, 2025 | Verify | Posted compensation ranges required. |
| Maine | Verify | Verify | Hiring-range disclosure requirement; confirm scope. |
| Virginia | Verify | Verify | Recently enacted/expanded; confirm current scope. |
| Massachusetts | **Oct 29, 2025** | **25+ employees** | Range required in postings; job postings must state whether commission is part of compensation. |
| District of Columbia | 2025 (delayed from earlier act) | Verify | Implementation date shifted; confirm current status. |
...
A fully-remote posting that could be performed from CO, WA, NY, CA, or similar generally must include a range compliant with those states' rules **regardless of where the employer is headquartered**. Practical effect: most national remote postings should carry a range.
...
These bans typically prohibit: asking the applicant's prior salary, screening applicants based on pay history, and (in many states) relying on salary history to set pay — with an exception if the applicant **volunteers** it unprompted. Some states also require disclosure of the range **on request** even without a posting law.

## How to use this file

1. Identify the state(s): employer HQ **and** any state the role (including remote) can be performed from.
2. Look up the posting-range rule and threshold above.
3. **Verify against current law** before making a claim to the user — cite the state agency page, not this file.
4. If a recruiter asks salary history, check the state against the ban list before characterizing the ask as legal or illegal; when unsure, the refusal script in the parent skill applies regardless.
